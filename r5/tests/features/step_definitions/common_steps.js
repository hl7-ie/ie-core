...
Given('the SUSHI compiler has been run successfully', function () {
  expect(this.isFshGeneratedAvailable(), 'fsh-generated directory not found. Run: sushi r5/').to.be.true;
});
...
Then('the profile fhirVersion should be {string}', function (version) {
  expect(this.resource.fhirVersion).to.equal(version);
});

Then('every profile fhirVersion should be {string}', function (version) {
  for (const profile of this.profiles) {
    expect(profile.fhirVersion, `Profile ${profile.id} has fhirVersion "${profile.fhirVersion}"`).to.equal(version);
  }
});
